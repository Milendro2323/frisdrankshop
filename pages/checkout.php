...
// Check of klant is ingelogd, anders doorsturen naar login
if (!is_customer_logged_in()) {
    header("Location: ?page=customer_login&redirect=checkout");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'place') {
    if (!hash_equals($_SESSION['csrf'] ?? '', $_POST['csrf'] ?? '')) {
        ...
        if ($first_name==='' || $last_name==='' || $email==='' || !filter_var($email, FILTER_VALIDATE_EMAIL)
            ...
            try {
                // Order opslaan MET user_id
                q(
                    $conn,
                    "INSERT INTO orders (user_id,first_name,last_name,email,address,postal_code,city,country,total,created_at)
                     VALUES (?,?,?,?,?,?,?,?,?,NOW())",
                    [$user_id,$first_name,$last_name,$email,$addr,$zip,$city,$country,$total]
                );
                ...
                $conn->commit();
                $_SESSION['cart'] = [];
                $success = "Bedankt voor je bestelling #$order_id!";
            } catch (Throwable $e) {
                $conn->rollback();
  ...
  <?php else: ?>
    ...
    <?php else: ?>
      ...
      <form method="post" class="form">
        ...
        <label>E-mail
          <input class="input" type="email" name="email" required value="<?= htmlspecialchars($_POST['email'] ?? $_SESSION['user_email'] ?? '') ?>">
        </label>
        <label>Adres
        ...
        <label>Land
          <input class="input" type="text" name="country" required value="<?= htmlspecialchars($_POST['country'] ?? 'Nederland') ?>">
        </label>
